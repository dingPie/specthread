...
| 규칙 ID | 의미 | 해결 |
|---------|------|------|
| `toc/stale` | 목차가 헤딩과 어긋남 | `npx specthread gen --write` |
| `reference/stale` | 참조 절이 본문 링크와 어긋남 | `npx specthread gen --write` |
| `pending/section-stale` | 미확정 사항 절이 관리 문서와 어긋남 | `npx specthread gen --write` |
| `heading/tail-name-reused` | 본문 절 제목이 꼬리 절 이름과 같음 | 절 제목 변경 |
| `section-ref/not-linked` | 다른 문서 절 참조에 앵커 링크가 없음 | 앵커 링크로 변환 |
| `marker/orphan` | 마커를 가리키는 관리 문서 항목 없음 | `specthread/pending.md`에 항목 추가 |
...
3. 지적이 있으면:
   - `toc/stale`, `reference/stale`, `pending/section-stale`은 `npx specthread gen --write`로 한번에 해결할 수 있다고 안내한다.
